...
def render_unified_profile_form():
    ...
    with st.form(key="unified_profile_form"):
        ...
        with col2:
            ...
            nut_dietary_preference = st.selectbox("Dietary Preference", options=["Non Vegetarian", "Vegetarian", "Vegan", "Pescatarian", "No Pork", "No Beef", "Halal", "Kosher"], index=0, key="nut_diet_pref")
            nut_dietary_restrictions = st.multiselect("Dietary Restrictions", options=["Gluten-free", "Dairy-free", "Lactose intolerant", "Low-sodium", "Low-fat", "Low-carb", "Sugar-free", "Paleo", "FODMAP", "No red meat"], key="nut_diet_rest")
            
            nut_medical_conditions = st.multiselect("Medical Conditions", options=MEDICAL_CONDITIONS_NUTRITION_OPTIONS, key="nut_med_cond")
            nut_digestive_issues = st.multiselect("Digestive Issues", options=DIGESTIVE_ISSUES_OPTIONS, default=["None"], key="nut_dig_issues")
            nut_cuisine = st.multiselect("Preferred Cuisine", options=CUISINE_OPTIONS, default=["None"], key="nut_cuisine")
    ...
    if nut_submitted:
        with st.spinner("Creating your profile and generating a personalized meal plan..."):
            ...
            profile_data = {
                ...
                "medical_conditions": [mc for mc in nut_medical_conditions if mc != "None"],
                "allergies": [a for a in nut_food_allergies if a != "None"],
                "cuisine": [c for c in nut_cuisine if c != "None"], 
                "vitals_numeric": {
                    "Heart Rate": nut_hr, 
            ...
            if blood_report_path:
                profile_data["blood_report_path"] = blood_report_path

            st.session_state.current_constraints.update(profile_data)
            initial_prompt = "Create a meal plan for me based on my profile."
            
            api_payload = {
                "query": initial_prompt,
                "last_agent_context": {},
                "force_tool_type": "meal_plan_generator" 
            }
            ...
            if response_data and "data" in response_data:
                ...
                st.session_state.messages.append({
                    ...
                    "audio": ai_content.get("audio_base64"),
