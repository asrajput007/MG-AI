...
from helpers.utils import DYNAMIC_DB_NAME, MASTER_TABLE_DB_NAME, WEEKDAY_ORDER, _unsubscribe_html_response, calculate_comprehensive_tdee, constraints_match, convert_json_to_profile_for_dashboard_message, extract_date_from_day, extract_food_data_from_mistral_analysis, extract_plan_blocks, fetch_chat_history, fetch_chat_history_v2, fetch_patient_profile, fetch_profile_data, generate_long_sas_url, generate_profile_hash, get_all_chat_titles, \
    ...
    get_previous_day_meal_plan_text, get_rotating_tips, get_user_profile, get_user_profile_v2, get_week_range, insert_vital_if_allowed, is_failed_response, is_sas_expired, json_serializer, log_failed_query, log_notification, normalize_payload, normalize_vital_name, normalize_weekly_plan, parse_items_to_meal_plan_text, parse_meal_plan_text_from_db, parse_meal_plan_text_ios, \
    parse_meal_plan_text_to_items, process_resubscribe, process_unsubscribe, save_tdee_goal, serialize_all, serialize_data, split_weekly_meal_plan, upsert_chat_history, upsert_weekly_grocery_list, validate_profile_for_meal_plan, generate_sas_url
...
@app.get("/")
def read_root():
    return {"message": "Welcome to the MG Testing API!"}

@app.get("/expert-notes")
async def get_expert_notes(
    request: Request,
    ...
    try:
        ...
        payload_jwt = get_jwt_payload(request)
        user_id = payload_jwt.get("Id")
        database_name = payload_jwt.get("DataBaseName")

        if not user_id or not database_name:
            raise HTTPException(status_code=400, detail="Invalid token")

        # ✅ DB Connection (your existing method)
        conn = get_db_connection_dynamic(database_name)
        cursor = conn.cursor()

        # ✅ Execute SP
        cursor.execute(
            "EXEC usp_GetEncounterLatestNotesByPatientId @PatientId=?",
            user_id
        )
        ...
        if not row:
            return {"message": "No expert notes found"}
...
# =========================================================
# MEAL PLAN FROM FRONTEND-SUPPLIED PROFILE (NO DB LOOKUP)
# =========================================================
@app.post("/generate-meal-plan-from-profile")
async def generate_meal_plan_from_profile(
    body: FrontendProfileMealPlanRequest,
    authorization: Optional[str] = Header(None)
):
    """
    Generates a meal plan directly from the profile submitted by the
    frontend (frontend1.py nutrition form), bypassing the DB profile
    lookup and DB-backed authorization used by /generate-weekly-meal-plan.
    A bearer token is still required as a lightweight identity marker, but
    it is never used to query the database for the profile.
    """
    scheme, _, token = (authorization or "").partition(" ")
    if scheme.lower() != "bearer" or not token:
    ...
    required_fields = ["name", "age", "gender", "weight_kg", "height_cm"]
    missing_fields = [f for f in required_fields if not profile.get(f)]
    if missing_fields:
        return JSONResponse(
            status_code=400,
            content={"message": f"Missing required profile fields: {missing_fields}"}
        )

    goal_type = (profile.get("goal") or {}).get("type", "Weight Maintenance")
    ...
    constraints = profile
    constraints["tdee"] = tdee
    user_id = body.user_id or constraints.get("user_id") or "frontend_user"
    constraints["user_id"] = user_id

    request_obj = ProcessQueryRequest(
        query=body.query or "Create a meal plan for me based on my profile.",
        chat_history=body.chat_history or [],
        current_constraints=constraints,
        last_agent_context=body.last_agent_context or {},
        force_tool_type=body.force_tool_type or "meal_plan_generator",
    ...
    return JSONResponse(status_code=200, content=serialize_data({"status": 200, "data": ai_data}))
...
async def generate_weekly_meal_plan(
    ...
    try:
        ...
        if grocery_items:
            ...
            grocery_start_date, grocery_end_date = get_week_range(start_date)

            upsert_weekly_grocery_list(
                cursor=cursor,
                user_id=user_id,
                start_date=grocery_start_date,
                end_date=grocery_end_date,
...
# =========================================================
# OPTIMIZED WEEKLY Swap MEAL PLAN API
# =========================================================
@app.post("/generate-weekly-swap-meal-plan")
async def generate_weekly_swap_meal_plan(
    request: Request,
    body: WeeklyMealPlanRequest,
    authorization: Optional[str] = Header(None)
):

    logger.info("==== Weekly SwapMeal Plan API Started ====")
    ...
    try:
        ...
        # cursor.execute("""
        #     SELECT DISTINCT MealDate
        #     FROM AlternativeMealPlanDetails WITH (NOLOCK)
        #     WHERE PatientId=?
        #     AND MealDate BETWEEN ? AND ?
        # """, (
        #     user_id,
        ...
        if profile_changed:
            ...
            cursor.execute("""
                SELECT DISTINCT
                    mpd.MealType
                FROM UserConsumedMeals ucm WITH (NOLOCK)
                INNER JOIN AlternativeMealPlanDetails mpd WITH (NOLOCK)
                    ON mpd.Identifier = ucm.UserMealItemId
                WHERE ucm.UserId = ?
                AND ucm.ConsumptionDateOnly = ?
                AND ISNULL(ucm.WasCompleted, 0) = 1
    ...
    should_regenerate = (
        profile_changed or bool(missing_dates) or getattr(body, 'force_regenerate', False)
    )

    logger.info(f"Should regenerate: {should_regenerate} (force={getattr(body, 'force_regenerate', False)})")
    ...
    for day_block in day_blocks:
        ...
        # Skip already existing dates (only when profile hasn't changed AND not force_regenerate)
        force_regen = getattr(body, 'force_regenerate', False)
        if not profile_changed and not force_regen and meal_date in existing_dates:
    ...
    try:
        ...
        if profile_changed or getattr(body, 'force_regenerate', False):

            logger.info(
                "Deleting existing meals (profile_changed=%s, force_regenerate=%s)" % (profile_changed, getattr(body, 'force_regenerate', False))
            )
            ...
            if today_consumed_meals:
                ...
                query = f"""
                    DELETE FROM AlternativeMealPlanDetails
                    WHERE PatientId = ?
                    AND MealDate = ?
                    AND MealType NOT IN ({placeholders})
                """
